...
func MarkVersionFailed(
	...
	if err != nil {
		return fmt.Errorf(
			"não foi possível registrar a falha da versão %d: %w",
			versionID,
			err,
		)
	}
	...
	if err != nil {
		return fmt.Errorf(
			"não foi possível confirmar a falha da versão %d: %w",
			versionID,
			err,
		)
	}
	...
	if rowsAffected != 1 {
		return fmt.Errorf(
			"versão %d não está em carga para registrar falha",
			versionID,
		)
	}
